...
if (isset($_SESSION['idhod']) && isset($_SESSION['fnamehod'])) {
    $dep=$_SESSION['dephod'];
   $sql = "SELECT * FROM request INNER JOIN student ON student.id_student = request.id_student where student.dep_student='$dep' and state = 0 ORDER BY id_req DESC";  
...
<section  id="requests-section" class="page-section ">
...
<div class="home__title" style="font-size: 1.6rem;
  ...
  color:#0e0e0e8f; "></i><span style=" color: #8041f5ab;">Request Refused </span> </div><br>         
...
<div class="table__body">
   ...
   {  ?>
           <tr class="score-card">
              <td> <br><?=$co=$co+1;?> </td>
              <td><img src="img/<?=$row['p_comp']?>"></td>
              <td><br><?=$row['firstname_student']." ".$row['lastname_student']?></td>
              <td><br><?=$row['name_comp']?></td>
              <td><br><?=$row['intern_name']?></td>
              <td> <br>
                 <a href="view2.php?id=<?=$row['id_req']?>" data-section="profile"> <button class="rq-btn">view</button> </a>
